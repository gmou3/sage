...
def AllMatroids(n, r=None, type='all'):
    """
    Return an iterator of all matroids of certain number of elements (and, optionally, of specific rank and type).
    This collection is retrieved from Yoshitake Matsumoto's ``Database of Matroids``.

    INPUT:

        - ``n`` -- an integer (1 <= n <= 9). The number of elements of the matroids.
        - ``r`` -- (default: None) an integer (1 <= r <= n). The rank of the matroids.
        - ``type`` -- (default: ``all``) a string. The type of the matroids, either ``all``, ``simple``, or ``nonorientable``.
